...
{
  return (facio_token_t){ strlen(src), strdup(src), 0, 0, { 0, 0.0, "" }, type };
}
static inline facio_token_t facio_get_token_int(facio_token type, long int src)
{
  char buf[255];
  sprintf(buf, "%ld", src);
  return (facio_token_t){ strlen(buf), strdup(buf), 0, 0, { src, 0.0, "" }, type };
}
static inline facio_token_t facio_get_token_float(facio_token type, double src)
{
  char buf[255];
  sprintf(buf, "%lf", src);
  return (facio_token_t){ strlen(buf), strdup(buf), 0, 0, { 0, src, "" }, type };
}
static inline facio_token_t facio_get_token_string(facio_token type, const char* src)
{
  return (facio_token_t){ strlen(src), strdup(src), 0, 0, { 0, 0.0, strdup(src) }, type };
}
